...
plot <- df$parks %>%
  ...
  mutate(
    highlight = ifelse(city == "Fresno", "#1D8534", "grey"),
  ...
  annotate(geom = "text", x = 0, y = 150, label = 'park access in Fresno has increased by 32 percentage \npoints between 2012 and 2020 despite a $5 fall in \nspend per resident', family = "Courier", size = 2, hjust = 0, color = "#1D8534") +
  scale_x_continuous(
  ...
  labs(
    ...
    subtitle = "The inequality in access to parks appears to be shrinking despite a growing inequality in \nspend per resident",
